...
■主要機能
- テレメトリ収集
車両シミュレータ（Python/EC2）から、走行速度・座標・状態等のデータを100ms周期でAWS IoT Coreへ送信。

- 車両制御
  Next.jsで構築した管理画面から、MQTTプロトコルを介して車両へコマンドを送信。
  本プロジェクトでは、以下項目を操作可能
  ・自動車ドアロック、アンロック
  ・エアコン設定温度
...
■課題(今後の改善点)
- アプリケーションからのデータフェッチのwebsocket化  
  現在は1秒ごとにhttpリクエストを送っているが、リアルタイム応答、コストカットの点から変更すべき。
